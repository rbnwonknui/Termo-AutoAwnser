...
function checkForDuo(modal) {
    const duo = localStorage.getItem('duo');
    if (duo) {
        try {
            const duoData = JSON.parse(duo);
            if (duoData.state && Array.isArray(duoData.state) && duoData.state.length >= 2) {
                const solution1 = duoData.state[0].solution;
                const solution2 = duoData.state[1].solution;
...
function startDuoChecker(modal) {
    if (checkForDuo(modal)) {
        return;
    }
