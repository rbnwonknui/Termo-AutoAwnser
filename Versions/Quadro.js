function showSolutionNotification(solutions) {
    ...
    notification.innerHTML = `
        ...
        <div class="solution-text">Soluções: ${solutions.map(s => s.toUpperCase()).join(", ")}</div>
    `;
...
function updateModalWithSolution(modal, solutions) {
    if (modal && modal.shadowRoot) {
        const solutionElement = modal.shadowRoot.querySelector('.solution');
        if (solutionElement) {
            solutionElement.innerHTML = `Soluções de hoje: ${solutions.map(s => s.toUpperCase()).join(", ")}`;
            solutionElement.classList.remove('loading');
        }
    }
}

function checkForQuatro(modal) {
    const quatro = localStorage.getItem('quatro');
    if (quatro) {
        try {
            const quatroData = JSON.parse(quatro);
            if (quatroData.state && Array.isArray(quatroData.state) && quatroData.state.length >= 4) {
                const solutions = [
                    quatroData.state[0].solution,
                    quatroData.state[1].solution,
                    quatroData.state[2].solution,
                    quatroData.state[3].solution
                ];
                if (solutions.every(Boolean)) {
                    updateModalWithSolution(modal, solutions);
                    showSolutionNotification(solutions);
                    return true;
                }
            }
        } catch (e) {}
...
function startQuatroChecker(modal) {
    if (checkForQuatro(modal)) {
        return;
    }
